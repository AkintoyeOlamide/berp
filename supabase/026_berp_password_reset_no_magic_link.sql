-- Create the password-reset table AND functions.
-- Run this in the SQL editor (fixes: relation "berp_password_resets" does not exist):
-- https://supabase.com/dashboard/project/vfrgawklszvhddrvjjxj/sql/new
--
-- Gmail SMTP for Auth emails (forgot password / OTP). Save in:
-- https://supabase.com/dashboard/project/vfrgawklszvhddrvjjxj/auth/providers
-- Authentication → SMTP Settings (or Project Settings → Authentication):
--   Enable custom SMTP = on
--   Sender name        = BERP
--   Sender email       = olamide@vmoaeros.com
--   Host               = smtp.gmail.com
--   Port number        = 587
--   Username           = olamide@vmoaeros.com
--   Password           = Gmail App Password (do not commit it)
--   Minimum interval   = default
-- Leave SSL off for port 587 (STARTTLS).
--
-- Then Authentication → Email Templates → Magic Link AND Reset Password:
-- <p>Your BERP code is <strong>{{ .Token }}</strong></p>
-- Authentication → Providers → Email → OTP length → 4
...
select 'berp_password_resets ready' as status;
